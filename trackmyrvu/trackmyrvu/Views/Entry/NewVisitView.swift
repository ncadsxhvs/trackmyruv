...
// MARK: - Add Procedure View (Placeholder)

struct AddProcedureView: View {
    @Environment(\.dismiss) private var dismiss
    let onAdd: (String, String, String, Double) -> Void

    @State private var hcpcs = ""
    @State private var description = ""
    @State private var statusCode = "A"
    @State private var workRVU = ""

    var body: some View {
        NavigationStack {
            Form {
                Section("Procedure Code") {
                    TextField("HCPCS Code (e.g., 99213)", text: $hcpcs)
                        .textCase(.uppercase)
                }

                Section("Description") {
                    TextField("Procedure description", text: $description, axis: .vertical)
                        .lineLimit(2...4)
                }
                ...
                Section("RVU Details") {
                    Picker("Status Code", selection: $statusCode) {
                        Text("Active (A)").tag("A")
                        Text("Billable (B)").tag("B")
                        Text("Carrier (C)").tag("C")
                    }

                    TextField("Work RVU", text: $workRVU)
                        .keyboardType(.decimalPad)
                }

                Section {
                    Text("Tip: Search for procedures from the HCPCS database (coming soon)")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Add") {
                        if let rvu = Double(workRVU), !hcpcs.isEmpty, !description.isEmpty {
                            onAdd(hcpcs, description, statusCode, rvu)
                            dismiss()
                        }
                    }
                    .disabled(hcpcs.isEmpty || description.isEmpty || workRVU.isEmpty)
                }
            }
        }
    }
